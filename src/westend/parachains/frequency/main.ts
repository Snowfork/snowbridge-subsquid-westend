...
processor.run(
  new TypeormDatabase({
    ...
    stateSchema: "frequency_processor",
  }),
  async (ctx) => {
  ...
  if (instruction.__kind == "WithdrawAsset" && instruction.value.length > 1) {
    ...
    if (asset.fun.__kind == "Fungible") {
      ...
      if (
        ...
        // PNA
        toEthereumAsset = {
          address: findTokenAddress("westend", location),
          amount,
          location,
        };
      }
    ...
    instruction.__kind == "ReceiveTeleportedAsset" &&
    instruction.value.length > 0
  ) {
...
const matchEthereumBeneficiary = (instruction: V5Instruction): string => {
  if (
    (instruction.__kind == "InitiateReserveWithdraw" ||
      instruction.__kind == "DepositReserveAsset") &&
  ...
  for (let block of ctx.blocks) {
    for (let event of block.events) {
      if (event.name == events.polkadotXcm.sent.name) {
        ...
        if (events.polkadotXcm.sent.v164.is(event)) {
          rec = events.polkadotXcm.sent.v164.decode(event);
        } else {
        ...
        let transferedAsset = matchToEthereumAsset(instruction0);
        if (!transferedAsset) {
          let instruction2 = rec.message[2];
          transferedAsset = matchToEthereumAsset(instruction2);
          if (!transferedAsset) {
            throw new Error("to ethereum unkown token info");
          }
        }
        tokenAddress = transferedAsset.address;
        tokenLocation = transferedAsset.location;
        amount = transferedAsset.amount;
        ...
        let transferToEthereum = new TransferStatusToEthereum({
          ...
          sourceParaId: WestendFrequencyParaId,
  ...
  for (let block of ctx.blocks) {
    for (let event of block.events) {
      if (
        ...
        if (events.messageQueue.processed.v164.is(event)) {
          rec = events.messageQueue.processed.v164.decode(event);
        } else if (events.messageQueue.processingFailed.v164.is(event)) {
          rec = events.messageQueue.processingFailed.v164.decode(event);
        } else {
          throw Object.assign(new Error("Unsupported spec"), event);
        }

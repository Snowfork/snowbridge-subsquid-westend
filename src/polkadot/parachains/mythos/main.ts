...
const matchToEthereumAsset = (
  ...
  let ethereumAsset;
  if (instruction.__kind == "WithdrawAsset") {
    let asset = instruction.value[0];
    if (instruction.value.length > 1) {
      asset = instruction.value[1];
    }
  ...
  for (let block of ctx.blocks) {
    for (let event of block.events) {
      if (
        ...
        if (events.messageQueue.processed.v1000.is(event)) {
          rec = events.messageQueue.processed.v1000.decode(event);
        } else if (events.messageQueue.processingFailed.v1000.is(event)) {
          rec = events.messageQueue.processingFailed.v1000.decode(event);
        } else if (events.messageQueue.processingFailed.v1004.is(event)) {
          rec = events.messageQueue.processingFailed.v1004.decode(event);
        } else {
          throw Object.assign(new Error("Unsupported spec"), event);
        }

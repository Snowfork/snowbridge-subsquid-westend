...
processor.run(
  new TypeormDatabase({
    ...
    stateSchema: "polkadot_neuroweb_processor",
  }),
  async (ctx) => {
...
const matchToEthereumAsset = (
  ...
  if (
    ...
    instruction.value.length > assetId
  ) {
    let asset = instruction.value[assetId];
  ...
  for (let block of ctx.blocks) {
    for (let event of block.events) {
      if (
        ...
        if (events.messageQueue.processed.v145.is(event)) {
          rec = events.messageQueue.processed.v145.decode(event);
        } else if (events.messageQueue.processingFailed.v145.is(event)) {
          rec = events.messageQueue.processingFailed.v145.decode(event);
        } else {
          throw Object.assign(new Error("Unsupported spec"), event);
        }

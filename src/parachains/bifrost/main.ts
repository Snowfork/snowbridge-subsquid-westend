  ...
  for (let block of ctx.blocks) {
    for (let event of block.events) {
      if (
        ...
        if (events.messageQueue.processed.v10000.is(event)) {
          rec = events.messageQueue.processed.v10000.decode(event);
        } else if (events.messageQueue.processingFailed.v10000.is(event)) {
          rec = events.messageQueue.processingFailed.v10000.decode(event);
        } else if (events.messageQueue.processingFailed.v12001.is(event)) {
          rec = events.messageQueue.processingFailed.v12001.decode(event);
        } else {
          throw new Error("Unsupported spec");
        }

  ...
  for (let block of ctx.blocks) {
    for (let event of block.events) {
      if (event.name == events.polkadotXcm.sent.name) {
        ...
        if (events.polkadotXcm.sent.v2250.is(event)) {
          rec = events.polkadotXcm.sent.v2250.decode(event);
        } else {
  ...
  for (let block of ctx.blocks) {
    for (let event of block.events) {
      if (
        ...
        if (events.messageQueue.processed.v2240.is(event)) {
          rec = events.messageQueue.processed.v2240.decode(event);
        } else if (events.messageQueue.processingFailed.v2240.is(event)) {
          rec = events.messageQueue.processingFailed.v2240.decode(event);
        } else if (events.messageQueue.processingFailed.v2270.is(event)) {
          rec = events.messageQueue.processingFailed.v2270.decode(event);
        } else {
          throw new Error("Unsupported spec");
        }
